...
一般的な「80%以上」といった数値を絶対的な目標（ノルマ）とするのではなく、**アーキテクチャの責務（層）に応じた濃淡をつける**ことがベストプラクティスです。

*   **🎯 90〜100% を目指す領域 (Domain Models / Application Service)**
    *   システムの中核となるビジネスロジック（金額計算、ステータス遷移の条件、在庫引き当てなど）。
    *   これらはバグが致命傷になるため、条件分岐（C1カバレッジ）も含めて、徹底的にユニットテストで保護します。
*   **🟢 70〜80% の目安で十分な領域 (Controller / Presentation)**
    *   Request/Responseのマッピングや、バリデーションエラーの分岐など。
    *   異常系（400エラーなど）の主要パターンが網羅されていれば十分とし、過度なテストは避けます。
*   **🟡 カバレッジが低くても許容される領域 (Infrastructure / External Client)**
    *   外部システム通信やDBアクセス層。
    *   ここはロジックの網羅性よりも、「繋がるかどうか」が重要です。正常系・異常系の代表的な結合テスト（`WireMockTest` や `@DataJpaTest`）が通っていればOKと判断し、すべての例外処理の網羅をユニットテストで無理に追及することは避けます。

数値目標はあくまで**「テストの書き忘れに気づくためのセンサー（低下時のアラート）」**として利用し、真の目的である「仕様上の分岐をすべてテストしているか」にフォーカスします。
